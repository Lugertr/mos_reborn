...
export class FileConversionService {
  ...
  async onSelectFiles(ev: Event): Promise<FileData[]> {
    ...
    if (isFilledArray(correctImgData)) {
      this.informerSrv.info(
        `Успешно загрузилось ${correctImgData.length} файлов`
      );
    } else {
      this.informerSrv.warn(
        'Не удалось загрузить ни одного файла, попробуйте повторить попытку'
      );
    }
  ...
  private async handleFileUpload(file: File | null): Promise<FileData> {
    try {
      ...
      if (tooLarge) {
        throw new Error(
          `Файл ${file.name} слишком тяжелый, максимальный размер файла: 10 МБ`
        );
      }
